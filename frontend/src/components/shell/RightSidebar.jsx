...
// Modern, consent-focused right sidebar with real data from API
// =======================================================================
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { Bell, Check, X, ShieldCheck, UserPlus, Image as ImageIcon, AlertCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchSidebarData = async () => {
      ...
      try {
        ...
        setConsentRequests(pending.slice(0, 3)); // Show only top 3

        // Fetch user suggestions
        const usersRes = await api.get('/api/users/');
        const otherUsers = usersRes.data.filter(u => u.username !== user.username);
        setSuggestions(otherUsers.slice(0, 3)); // Show only top 3
      } catch (error) {
    ...
    <aside className="bg-surface border-l border-gray-200 w-80 fixed top-0 right-0 h-full z-30 hidden lg:flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 h-16 border-b border-gray-200">
        ...
        <button 
          ...
          className="p-2 rounded-full hover:bg-background relative transition-colors"
        >
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto">
        {/* Consent Requests Section - Priority #1 */}
          ...
          {loading ? (
            ...
            consentRequests.map(request => (
              <ConsentRequestItem 
                key={request.id}
                request={request}
                onApprove={() => handleConsentAction(request.id, 'APPROVED')}
                onDeny={() => handleConsentAction(request.id, 'DENIED')}
              />
            ))
          ) : (
        ...
        <div className="bg-background p-4 rounded-lg">
          <h3 className="font-semibold text-sm text-gray-700 mb-3">Your Activity</h3>
          <div className="space-y-2">
            <StatItem icon={<ImageIcon className="w-4 h-4" />} label="Photos Shared" value="12" />
            <StatItem icon={<ShieldCheck className="w-4 h-4" />} label="Consents Given" value="8" />
            <StatItem icon={<UserPlus className="w-4 h-4" />} label="Connections" value="24" />
          </div>
        </div>
      </div>
...
const ConsentRequestItem = ({ request, onApprove, onDeny }) => {
  const uploader = request.photo?.uploader;
  
  return (
    <div className="bg-background p-3 rounded-lg border border-gray-200 hover:border-primary/30 transition-colors">
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gray-600 mb-2">Tagged you in a photo</p>
          <div className="flex space-x-2">
            <button 
              onClick={onDeny}
              className="flex-1 flex items-center justify-center space-x-1 px-2 py-1.5 text-[11px] font-semibold text-red-600 bg-red-50 hover:bg-red-100 rounded-md transition-colors"
            >
              <X className="w-3 h-3" />
              <span>Deny</span>
            </button>
            <button 
              onClick={onApprove}
              className="flex-1 flex items-center justify-center space-x-1 px-2 py-1.5 text-[11px] font-semibold text-white bg-primary hover:bg-dark-accent rounded-md transition-colors"
            >
